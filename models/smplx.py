...
class WrappedSMPLX(WrappedBASE):
    ...
    def __init__(self, name_or_path, outputs_dir, device, **kwargs) -> None:
        ...
        h, w = kwargs.get('h', 512), kwargs.get('w', 512)
        ...
        self.renderer = pyrender.OffscreenRenderer(viewport_height=h, viewport_width=w)
    ...
    def call(self, **kwargs):
        ...
        current_save_dir = self.outputs_dir / '_'.join(str(data_dir).split('/')[-3:])
        current_save_dir.mkdir(parents=True, exist_ok=True)

        with (data_dir/'smplx.npy').open('rb') as f:
